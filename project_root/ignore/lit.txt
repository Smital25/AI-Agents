...
# Suppress excessive logging from torch and sentence_transformers
import logging
...
def extractive_summary(text: str, top_k_sentences: int = 3) -> str:
    ...
    try:
        emb_doc = cast(torch.Tensor, embedder.encode(txt, convert_to_tensor=True))
        sent_embs = cast(torch.Tensor, embedder.encode(sentences, convert_to_tensor=True))
...
def rank_by_relevance(papers: List[Dict[str, Any]], query: str) -> List[Dict[str, Any]]:
    ...
    for p in candidates:
        ...
        try:
            emb = cast(torch.Tensor, embedder.encode(combined, convert_to_tensor=True))
